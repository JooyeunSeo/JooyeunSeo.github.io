...
d = x.remove(2)         # x = [1, 3, 4]            (가장 앞에 있는 해당 값 꺼내기)
print(d)                # None   (반환값 없음!)

e = x.reverse()         # x = [4, 3, 1]            (거꾸로 뒤집기)
print(e)                # None   (반환값 없음!)

f = x.sort()            # x = [1, 3, 4]            (오름차순 정렬)
print(f)                # None   (반환값 없음!)
f = sorted(x)           
print(f)                # [1, 3, 4]
```
